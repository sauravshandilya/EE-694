      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (strlen(buffer) == 0) {
            ...
            //Yellow LED
            client.print("YELLOW LED <button onclick=\"location.href='/YH'\">HIGH</button>");
            client.println(" <button onclick=\"location.href='/YL'\">LOW</button><br><br>");

            //Green LED
            client.print("GREEN LED <button onclick=\"location.href='/GH'\">HIGH</button>");
            client.println(" <button onclick=\"location.href='/GL'\">LOW</button><br><br>");
        ...
        {
          digitalWrite(RED_LED, HIGH);               // GET /RH turns the Red LED ON
        }
        if (endsWith(buffer, "GET /RL")) 
        {
          digitalWrite(RED_LED, LOW);                // GET /RL turns the Red LED OFF
        }

        if (endsWith(buffer, "GET /YH"))             
        {
          digitalWrite(YELLOW_LED, HIGH);                // GET /YH turns the Yellow LED ON
        }

        if (endsWith(buffer, "GET /YL")) 
        {
          digitalWrite(YELLOW_LED, LOW);                // GET /YL turns the Yellow LED OFF
        }

        if (endsWith(buffer, "GET /GH"))             
        {
          digitalWrite(GREEN_LED, HIGH);                // GET /GH turns the Green LED ON
        }

        if (endsWith(buffer, "GET /GL")) 
        {
          digitalWrite(GREEN_LED, LOW);                // GET /GL turns the Green LED OFF
        }
      }
    }
